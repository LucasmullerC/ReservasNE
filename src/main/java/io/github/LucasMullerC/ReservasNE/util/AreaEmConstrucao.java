...
public class AreaEmConstrucao
{
    private File storageFile;
    private ArrayList<Areas> values;
    private Areas A;
    
    public AreaEmConstrucao(final File file) {
            ...
            while ((line = reader.readLine()) != null) {
                if (!this.Contains(new Areas(line))) {
                    if (line.charAt(0) == '<' && line.charAt(1) == 'n') {
                        ...
                        nome = nome[1].split("<");
                        final String difi = nome[0].replaceAll("\\D+", "");
                        (this.A = new Areas(nome[0])).setDificuldade(String.valueOf(difi.charAt(0)));
                    }
                    else if (line.charAt(0) == '<' && line.charAt(1) == 'd') {
                        String[] dif = line.split(">");
                        dif = dif[1].split("<");
                        final String[] letras = dif[0].split("\\s+");
                        this.A.setCredito(letras[3]);
                    }
                    else {
                        if (line.charAt(0) != '-') {
                            continue;
    ...
    public void save() {
        try {
            ...
            for (final Areas value : this.values) {
                ...
                out.newLine();
                out.write("<description>Em constru\u00e7\u00e3o por: " + value.getCredito() + "</description>");
                out.newLine();
