...
public class AreaEmConstrucao {
	...
	public AreaEmConstrucao(File file) {
			...
			while ((line = reader.readLine()) != null) {
				if (this.Contains(new Areas(line)) == false) {
					if (line.charAt(0) == '<' && line.charAt(1) == 'n') {
						...
						nome = nome[1].split("<");
						String difi = nome[0].replaceAll("\\D+","");
						A = new Areas(nome [0]);
						A.setDificuldade(String.valueOf(difi.charAt(0)));
					}
					else if (line.charAt(0) == '<' && line.charAt(1) == 'd'){
						String[] dif = line.split(">");
						dif = dif[1].split("<");
						String[] letras = dif[0].split("\\s+");
						A.setCredito(letras[3]);	
					}
